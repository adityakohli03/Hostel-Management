...
public class Update_Delete_Employee extends JFrame implements ActionListener {
	...
	Update_Delete_Employee(){
       ...
       b5.setIcon(new ImageIcon(i2));
	    ...
	    //----------mobile---------- 
		 ...
		 salary.setBounds(40,450,100,30); 
		 f.add(salary);
		 
		 tf9=new JTextField();
		 tf9.setBounds(180,450,370,30);
		 f.add(tf9);
		 
		 
		 //---------status--------
		 status=new JLabel("Status");
		 status.setBounds(40,500,100,30); 
		 f.add(status);
		 
		 
		  cmb=new JComboBox<String>();
		  cmb.setBounds(180,500,370,30);
		  f.add(cmb);
		...
		if(e.getSource()==b1) {
			...
			try {
			 ...
			 if(rs.next())
			 {
				 ...
				 tf9.setText(String.valueOf(rs.getInt(10)));
				 tf8.setEditable(false);
				 if(rs.getString(9).equals("Working"))
				 {
					 cmb.addItem("Working");
					 cmb.addItem("Not Working");
				 }
				 else 
				 {
					 cmb.addItem("Not Working");
			...
			String salary=tf9.getText();
			String status=(String)cmb.getItemAt(cmb.getSelectedIndex());
   ...
   String query = "update employee set Name='"+name+"',Father_Name='"+fname+"',Mother_Name='"+mname+"',Email='"+email+"',Address='"+address+"',Aadhar='"+aadhar+"',Status='"+status+"',Salary='"+salary+"' where Mobile_Number='"+mobile+"'";
			...
			try {
			 conn con=new conn();
			   String query = "delete from employee where Mobile_Number='"+tf1.getText()+"'";
			   con.s.execute(query);
