...
public class Employee_Leaved extends JFrame implements ActionListener{
	JLabel heading;
	JPanel contentPane;
	 JTable table;
	// int x=1;
     ...
     String[] columnNames={"Name","Father Name","Mother Name","Mobile","Email", "Address","Aadhar No","Designation","Salary"};
	Employee_Leaved(){
       ...
       b1.setIcon(new ImageIcon(i2));
		...
		//------------heading----------
		heading=new JLabel("Employee Leaved");
		...
		try {
		conn con=new conn();
		 ResultSet rs=con.s.executeQuery("select * from employee where Status='Not Working'");
