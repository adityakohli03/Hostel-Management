...
public class Student_Fee extends JFrame implements ActionListener {
	...
	JLabel mobile,name,email,roomnumber,payment,month;
	JButton b1,b2,b3,b4;
	//int x=1;
	...
	public Student_Fee() {
       ...
       b4.setIcon(new ImageIcon(i2));
		...
		roomnumber.setFont(new Font("Tahoma", Font.BOLD, 13));
		roomnumber.setBounds(40, 200, 100, 30);
	...
	try {
	 conn con=new conn();
	 ResultSet rs=con.s.executeQuery("select * from student_fee where Mobile_Number='"+mobile+"'");
	while(rs.next()) {
		...
		if(e.getSource()==b1) {
			...
			try {
			 ...
			 if(rs.next())
			 {
				 ...
				 {
					 JOptionPane.showMessageDialog(null,"Student Leaved The University");
					 clear();
				 }
			...
			ResultSet rs2=con.s.executeQuery("select * from student_fee inner join student where student.Status='Living' and student_fee.Month='"+my+"' and student_fee.Mobile_Number='"+mobile+"'");
			if(rs2.next()) {
				b1.setEnabled(false);
				b2.setVisible(false);
				table_details();
			...
			try {
				conn con=new conn();
			con.s.executeUpdate("insert into student_fee(Mobile_Number,Month,Amount) values('"+mobile+"','"+month+"','"+amount+"')");
				table_details();
